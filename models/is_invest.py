...
class IsInvestGlobal(models.Model):
    ...
    site_id       = fields.Many2one("is.database", "Site", select=True,required=True)
...
class IsInvestDetail(models.Model):
    ...
    date_creation = fields.Date(u"Date de création", copy=False, default=fields.Date.context_today, readonly=True)
    annee         = fields.Char(u'Année', select=True, required=True, default=lambda *a:time.strftime('%Y'))
    site_id       = fields.Many2one("is.database", "Site", select=True,readonly=True)
    intitule      = fields.Char(u'Intitulé', required=True)
    imputation    = fields.Char(u'Imputation', required=True, select=True)
...
class IsInvestCde(models.Model):
    ...
    detail_id     = fields.Many2one("is.invest.detail", "Invest Détail", select=True)
    order         = fields.Char(u'N°Cde', select=True)
    order_id      = fields.Integer(u'N°Cde Id')
    code_pg       = fields.Char(u'Code PG', select=True)
    ...
    base          = fields.Char(u'Base', select=True)
    #site_id       = fields.Many2one("is.database", "Site", select=True)
...
class IsInvestCompta(models.Model):
    ...
    detail_id     = fields.Many2one("is.invest.detail", "Invest Détail", select=True)
    date_facture  = fields.Date(u'Date', select=True)
